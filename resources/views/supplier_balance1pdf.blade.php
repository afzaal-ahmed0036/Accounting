...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td colspan="2"><div align="center"><span class="style2">{{session::get('CompanyName')}}</span></div></td>
  </tr>
  <tr>
...
$total_m =0;
   ...
   while (strtotime($start_date2) <= strtotime($end_date2)) { ?>

<?php 



$grand_monthly_bal = DB::table('v_supplier_balance')
                      ->select( DB::raw('sum(if(ISNULL(Dr),0,Dr)-if(ISNULL(Cr),0,Cr)) as Balance'))
                      ->where('Date', date("M-Y",strtotime($start_date2))  )
 ...
 ?>
    ...
    <td align="right">{{number_format($grand_opening_balance[0]->Balance+$GrandTotal[0]->Balance,2)}}</td>
  </tr>
